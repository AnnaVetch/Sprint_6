...
class TestMainPage:
    ...
    @allure.title("Тест- переход по логотипу Скутер")
    def test_click_scooter_logo_success(self,driver):
    ...
    @allure.title("Тест- переход по логотипу Яндекс")
    def test_click_yandex_logo_success(self,driver):
        main_page = MainPage(driver)
        main_page.open_page()
    ...
    def test_faq(self,driver, question_locator, answer_locator, expected_answer):
        main_page = MainPage(driver)
        main_page.open_page()
